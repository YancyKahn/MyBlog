...
class SubscribeView(LoginRequiredMixin,AuthorRequiredMixin, generic.UpdateView):
    ...
    template_name = 'user/subscribe.html'

    def get_success_url(self):
        messages.success(self.request, "保存成功")
        return reverse('user:subscribe', kwargs={'pk': self.request.user.pk})
...
class FeedbackView(LoginRequiredMixin, generic.CreateView):
    ...
    def post(self, request, *args, **kwargs):
        ...
        if was_limited:
            ...
            return render(request, 'user/feedback.html', {'form': FeedbackForm()})
        return super().post(request, *args, **kwargs)

    def get_success_url(self):
        messages.success(self.request, "提交成功")
        return reverse('user:feedback')
...
class LikeListView(generic.ListView):
    model = User
    template_name = 'user/like_videos.html'
    context_object_name = 'video_list'
    paginate_by = 10
